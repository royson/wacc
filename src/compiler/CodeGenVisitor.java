...
public class CodeGenVisitor extends WACCParserBaseVisitor<Void> {
    ...
    public Void visitAssignrhsexpr(WACCParser.AssignrhsexprContext ctx) {
        ...
        if(PASS == 2){
          if(stack.peek().startsWith("lhs")){
            lhs = true;
            String fstOrSnd = stack.pop();
            if(fstOrSnd.split("[,]")[1].equals("snd")){
          	offsetLHS = true;
            }
          }
        }
        ...
        visit(ctx.expr());
        String savedExprReg = null;
        if(PASS == 2 
        	&& lhs){
          savedExprReg = currentReg;
          lockReg();
        }
        printStack();
        String exprType = stack.pop();
        ...
        if (PASS == 2) {
            ...
            if (Utils.isArrayElem(varName)) {
              ...
              if (exprType.equals("CHAR")
            	...
            	text.add("STRB "+savedExprReg+", ["+currentReg+"]");
              } else{
            	text.add("STR "+savedExprReg+", ["+currentReg+"]");
              }
              checkNullPointer();
              releaseReg();
            } else {
